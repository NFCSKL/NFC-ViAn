...
void filehandlertest::project_handling_test(){
    ...
    fh->add_video(proj, fh->get_project(vid1)); //add tesfiles as videos to project.
    fh->add_video(proj, fh->get_project(vid2));
    fh->add_video(proj, fh->get_project(vid3));
...
void filehandlertest::file_test(){
    ...
    QCOMPARE(fh->get_file(dir), dirpath);
    QCOMPARE(fh->get_project(fileID), dirpath + "/" + filename);

    ID fileID2  = fh->create_file(filename,dir); // Create file ID = i+1 (2)
    QCOMPARE(fileID2 - fileID, 1);                         // check if 1 && 2 is true
    QVERIFY(fh->get_project(fileID) == fh->get_project(fileID2)); //check that (1), (2) point to same file.
